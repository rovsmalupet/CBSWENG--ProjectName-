...
export default function EditProject() {
  ...
  const [form, setForm] = useState({
    campaignTitle: "", location: "", description: "",
    monetarySupport: "", volunteerQuantity: "", priority: "",
  ...
  const [supportTypes, setSupportTypes] = useState({ monetary: false, inKind: false, volunteer: false });
  ...
  const handleCancelUpdate = () => { setShowConfirmation(false); setPendingUpdate(null); };
  ...
  return (
    <div className="postProjectPage">
      <div className="postProjectCard">
        ...
        {status === "success" && <div className="postProjectAlertSuccess">✅ Project updated successfully! Redirecting...</div>}
        ...
        {showConfirmation && (
          <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, backgroundColor: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }}>
            <div style={{ backgroundColor: "white", borderRadius: "8px", padding: "24px", maxWidth: "400px", boxShadow: "0 4px 6px rgba(0,0,0,0.1)" }}>
              <h3 style={{ marginTop: 0, marginBottom: "12px", fontSize: "18px" }}>⚠️ Status Change Notice</h3>
              <p style={{ marginBottom: "20px", color: "#555", lineHeight: "1.5" }}>
                When you edit this project, its status will be changed to <strong>Edited</strong>. Your project will need to be reviewed and approved by an admin again before it becomes active.
              </p>
              <div style={{ display: "flex", gap: "12px", justifyContent: "flex-end" }}>
                <button onClick={handleCancelUpdate} style={{ padding: "8px 16px", borderRadius: "4px", border: "1px solid #ccc", backgroundColor: "#f5f5f5", cursor: "pointer", fontWeight: "500" }}>Cancel</button>
                <button onClick={handleConfirmUpdate} style={{ padding: "8px 16px", borderRadius: "4px", border: "none", backgroundColor: "#2563eb", color: "white", cursor: "pointer", fontWeight: "500" }}>Continue Editing</button>
              </div>
            </div>
          </div>
        )}
